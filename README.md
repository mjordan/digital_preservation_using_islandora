...
Islandora does not currently perform normalization of ingested files into preservation-friendly formats. It does tranform many types of files into specific formats or characteristics for specific purposes; for example, if a user ingests a TIFF, Islandora will create a JPEG2000 version of that TIFF for presentation to the user. The technical architecture - microservices - to perform normalization on ingest, or to perform format migration at scale is present in Islandora. But to date, no one from the Islandora community has applied that architecture to preservation-friendly normalization. Given its use of microservices to transform content into presentation-friendly formats (e.g. TIFF to JPEG2000), Islandora has the potential to transform content into preservation-friendly formats. It is possible to extend Islandora to use Archivematica's [Format Policie Registry](https://www.archivematica.org/en/docs/archivematica-1.12/user-manual/preservation/preservation-planning/) or [Preservation Action Registries](https://parcore.org/) if the community defines that functionality as a priority.
